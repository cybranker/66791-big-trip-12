...
class TripEventEdit extends SmartView {
  constructor(userAction, offers, destinations, trip = BLANK_TRIP) {
    super();
    this._data = TripEventEdit.parseTripToData(trip);
  ...
  reset(trip) {
    this._offers = trip.offers.slice();
    this.updateData(
        TripEventEdit.parseTripToData(trip)
    );
  }
  ...
  _createFavoriteTemplate(isFavorite, isDisabled) {
    return `<input id="event-favorite-1" class="event__favorite-checkbox  visually-hidden" type="checkbox" value="${isFavorite}" name="event-favorite" ${isFavorite ? `checked` : ``} ${isDisabled ? `disabled` : ``}>
  ...
  _createEventOffersTemplate(offers, offersChecked, eventType, isDisabled) {
    let offersTemplate = ``;
      ...
      if (it.type === eventType && it.offers.length > 0) {
        const eventOffersTemplate = it.offers.map((offer, index) => {
          ...
          return `<div class="event__offer-selector">
            <input class="event__offer-checkbox  visually-hidden" id="event-offer-${eventType}-${index}" type="checkbox" value="${price}" data-title="${title}" name="event-offer-${eventType}-${index}" ${offersChecked.some((offerCheck) => offerCheck.title === title) ? `checked` : ``} ${isDisabled ? `disabled` : ``}>
  ...
  _createTripEventEditTemplate(data, allOffers, allDestinations) {
    ...
    const favoriteTemplate = isActionAddTrip ? `` : this._createFavoriteTemplate(isFavorite, isDisabled);
    const offersTemplate = this._createEventOffersTemplate(allOffers, offers, getEventWithoutActionName(event), isDisabled);
    ...
    const template = `<form class="${isActionAddTrip ? `trip-events__item` : ``} event  event--edit" action="#" method="post">
        <header class="event__header">
          ...
          <div class="event__field-group  event__field-group--destination">
            ...
            <input class="event__input  event__input--destination" id="event-destination-1" type="text" name="event-destination" value="${he.encode(city)}" list="destination-list-1" ${disabled}>
          ...
          <div class="event__field-group  event__field-group--time">
            ...
            <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${humanizeTaskDate(timeIn, `2-digit`).split(`, `).join(` `)}" ${disabled}>
            &mdash;
            <label class="visually-hidden" for="event-end-time-1">
              To
            </label>
            <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${humanizeTaskDate(timeOut, `2-digit`).split(`, `).join(` `)}" ${disabled}>
          </div>
          ...
          <button class="event__save-btn  btn  btn--blue" type="submit" ${!city || isDisabled ? `disabled` : ``}>${isSaving ? `Saving...` : `Save`}</button>
          <button class="event__reset-btn" type="reset" ${disabled}>${isActionAddTrip ? `Cancel` : `${isDeleting ? `Deleting...` : `Delete`}`}</button>
  ...
  _formSubmitHandler(evt) {
    evt.preventDefault();
    this._callback.formSubmit(TripEventEdit.parseDataToTrip(this._data));
  }
  ...
  _formDeleteClickHandler(evt) {
    evt.preventDefault();
    this._callback.deleteClick(TripEventEdit.parseDataToTrip(this._data));
  }
